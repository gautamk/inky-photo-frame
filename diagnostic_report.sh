...
OUTPUT_FILE="$HOME/inky_diagnostic_report_$(date +%Y%m%d_%H%M%S).txt"
...
ls -la "$HOME/Images/" >> $OUTPUT_FILE 2>&1
echo "Total images: $(find "$HOME/Images" -type f \( -name "*.jpg" -o -name "*.png" -o -name "*.heic" \) 2>/dev/null | wc -l)" >> $OUTPUT_FILE
echo "" >> $OUTPUT_FILE
echo "Application Directory:" >> $OUTPUT_FILE
ls -la "$HOME/inky-photo-frame/" >> $OUTPUT_FILE 2>&1
echo "" >> $OUTPUT_FILE
...
"$HOME/.virtualenvs/pimoroni/bin/pip" list >> $OUTPUT_FILE 2>&1
echo "" >> $OUTPUT_FILE
...
echo "---------------" >> $OUTPUT_FILE
if [ -f "$HOME/.inky_history.json" ]; then
    echo "History file exists:" >> $OUTPUT_FILE
    cat "$HOME/.inky_history.json" >> $OUTPUT_FILE 2>&1
else
